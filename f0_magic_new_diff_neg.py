...
def train_model(
    ...
    train_dataset = torch.utils.data.TensorDataset(
        train_others_data_p, train_others_data_d
    )
    if USE_TEST_SET:
        test_dataset = torch.utils.data.TensorDataset(
            test_others_data_p, test_others_data_d
        )
    else:
        if test_others_data:
            train_dataset += torch.utils.data.TensorDataset(
    ...
    while True:
        ...
        def work(data_p, data_d, is_train, losses):
            ...
            outputs_legacy = (
                postprocess_legacy(
                    model_legacy(
                        preprocess_legacy(data_p.unsqueeze(1), data_d.unsqueeze(1))
                    )
                )
                .squeeze(1)
                .detach()
...
if __name__ == "__main__":
    ...
    train_model(
        "neg", train_target_data, train_others_data, test_target_data, test_others_data
    )

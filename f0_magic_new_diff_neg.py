...
def f0_magic_log(s):
    while True:
        try:
            with open("f0_magic_new_neg.log", "a") as f:
                print(s, file=f)
            break
        except:
            pass
...
def train_model(name, train_target_data, train_others_data, test_target_data, test_others_data):
    ...
    train_dataset = torch.utils.data.TensorDataset(train_others_data_p, train_others_data_d)
    if USE_TEST_SET:
        test_dataset = torch.utils.data.TensorDataset(test_others_data_p, test_others_data_d)
    else:
        if test_others_data:
            train_dataset += torch.utils.data.TensorDataset(test_others_data_p, test_others_data_d)
    ...
    while True:
        ...
        def work(data_p, data_d, is_train, losses):
            if is_train:
                net_g.train()
            else:
                net_g.eval()
            data_p, data_d = data_p.to(device), data_d.to(device)
            ...
            inputs, ref = outputs_legacy, data_p
        ...
        train_loss = []
        for batch_idx, (data_p, data_d) in enumerate(train_loader):
            if batch_idx % 10 == 0:
                print(f"train {batch_idx}/{len(train_loader)}")
            work(data_p, data_d, True, train_loss)

        train_loss = np.mean(train_loss)

        if USE_TEST_SET:
            test_loss = []
            for batch_idx, (data_p, data_d) in enumerate(test_loader):
                if batch_idx % 10 == 0:
                    print(f"val {batch_idx}/{len(test_loader)}")
                work(data_p, data_d, False, test_loss)
...
#    np.random.seed(42)
    ...
    train_model("neg", train_target_data, train_others_data, test_target_data, test_others_data)

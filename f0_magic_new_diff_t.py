...
def preprocess(x, y, z):
    ...
    #    z_ret = (z - mn_p) / std_p
    z_ret = zero_sensative_blur(z)
    z_ret = z_ret + torch.randn_like(z_ret) * preprocess_noise_amp_p
    z_ret = (z_ret - mn_p) / std_p
...
def train_model(
    ...
    while True:
        ...
        def work(data_p, data_d, labels, is_train, losses):
            ...
            loss = F.mse_loss(
                outputs[:, padding_size:-padding_size],
                ref[:, padding_size:-padding_size],
            )

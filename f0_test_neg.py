...
from f0_magic_new_diff_neg import compute_f0_inference, compute_d, resize, pitch_invert_mel, pitch_shift_mel
from f0_magic_new_diff_neg import postprocess, preprocess, padding_size
from f0_magic_gen_diff import PitchContourGenerator, segment_size
from f0_magic_new_diff_neg import snap
from f0_magic_new_diff_neg import num_timesteps, sample
...
legacy_contour_mel_tensor = postprocess_legacy(model_legacy(preprocess_legacy(input_contour_mel_tensor.unsqueeze(0).unsqueeze(0), input_phone_diff_tensor.unsqueeze(0).unsqueeze(0)))).detach().squeeze(0).squeeze(0)
...
for t in reversed(range(num_timesteps)):
    ...
    modified_contour_mel_tensor = sample(model, modified_contour_mel_tensor.unsqueeze(0).unsqueeze(0), input_phone_diff_tensor.unsqueeze(0).unsqueeze(0), legacy_contour_mel_tensor.unsqueeze(0).unsqueeze(0), t_tensor).detach().squeeze(0).squeeze(0)
